...
export function ensureDictImported() {
  if (!importPromise) {
    importPromise = doImport().catch((error) => {
      ...
      importPromise = null;
      console.warn("[offlineDict] 词库导入失败：", error);
    });
  }
...
async function doImport() {
  ...
  db.close();
  console.log(`[offlineDict] 词库导入完成：${entries.length} 词条 / ${lemmaEntries.length} 词形`);
}
...
export async function lookupOffline(word) {
  ...
  try {
    ...
    console.warn("[offlineDict] 打开数据库失败：", error);
    return null;
  }
  ...
  try {
    ...
    console.warn("[offlineDict] 查询失败：", error);
    return null;
  } finally {
    db.close();
  }
